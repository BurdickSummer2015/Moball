...
#To get started make a folder on your computer that you are going to use to build everything. From now on we'll call it your "Phyflex" directory. Put it somewhere not too deep in your filesystem like in your home directory or Documents or Desktop. Make sure not to put your Phyflex directory inside a git/mercurial repository. This directory is going to be filled by several Gigabytes of data. When you start this setup script it will prompt you for your Phyflex Directory. Or if you prefer you can do the following:

#./Phyflex_i.MX6_Setup.sh -p=<path to your PhyflexDir>

#When you are comfortable doing so, go ahead and run this file to get started. This will take a very long time to run, and you really ought to be aware of what it is doing, so please by all means read this file at your leisure. The important bits are thoroughly commented upon.
...
#First and most importantly we need to use minicom. Minicom is a Serial Terminal. It is how you connect to the board via the RS-232/USB adapter you got earlier. Go ahead and connect the RS-232 end of the adapter to the UART 0 part of the board, and the USB end to your computer. Now run minicom in your terminal with:

#sudo minicom

#Now do CTRL+A, and then Z to open up the Minicom Command Summary. Then press O to Configure Minicom. Use the arrow keys to scroll down to "Serial port setup" and press Enter. The default configuration should be fine (115200 8N1, Hw Flow Yes, Sft Flow No) except that serial device should be changed to "/dev/ttyUSB0". Note that if you have other USB devices connected to you host machine then they may be hooked up to USB0 instead. So you may need to use "/dev/ttyUSB1". To change this just press "A" and then start typing. Then press Enter twice scroll down to "Save setup as dfl" and press Enter again. Then scroll down to "Exit" and press Enter. To Exit out of minicom press CTRL+A then X. Do this and then start minicom again with "sudo minicom". Note that if your host machine actually has an RS-232 port then you should use that instead of an adapter, and you probably won't need to alter the settings in minicom at all. 
...
#Now we can setup eclipse. Run eclipse by typing "eclipse &" from the terminal. Either pick a new workspace or use the default one. Either way, if you got this code from a git repository, then you should have cloned this repository into your workspace folder. If you have not done this then go ahead and move the whole repository to your workspace folder now. If you have a working project then go ahead and use it otherwise follow the following instructions for making a new eclipse project.
...
#where $BoardStaticIP is the static IP of your board. You should get a static IP from your network administrator to be safe. But you can always ping addresses in your network until you find an unused one. Although this will almost certainly cause issues later. Note that if you have netconfig.sh then you should simply use that, as it will do all of this automatically for you.

#Once you have a static IP, or just an unsued one, and have correctly configured the network settings on your board as outlined in section 8.2.1 of the quickstart guide got to Project->Properties->C/C++ Build->Settings->Build Steps, and in Post-build steps, in the Command: line add the following:

#scp ./<name of project> root@<The IP address you chose>:/home/. 

#And thats it for eclipse
