...
#Now we can setup eclipse. Run eclipse by typing "eclipse &" from the terminal. Either pick a new workspace or use the default one. Either way, if you got this code from a git repository, then you should have cloned this repository into your workspace folder. If you have not done this then go ahead and move the whole repository to your workspace folder now. If you have a working project then open it up with File->Import->Existing Project From workspace, and skip the next step. Otherwise follow the following instructions for making a new eclipse project.
